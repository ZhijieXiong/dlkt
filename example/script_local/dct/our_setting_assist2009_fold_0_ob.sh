...
#    done
#  done
#} >> F:/code/myProjects/dlkt/example/result_local/dct_baseline_our_setting_assist2009_fold_0_ob.txt
...

{
  ...
  ws_que_diff_pred='0'
  ws_que_disc_pred='0'
  ws_user_ability_pred='0'
  ws_penalty_neg='0'
  ws_learning='0'
  ws_counter_fact='1 3 5 10'
  for weight_decay in ${weight_decays}
  do
    for num_rnn_layer in "${nums_rnn_layer[@]}"
    do
        ...
        do
          ...
          do
            ...
            do
              ...
              do
                for w_user_ability_pred in ${ws_user_ability_pred}
                do
                  ...
                  do
                    ...
                    do
                      ...
                      do
                        echo -e "weight_decay: ${weight_decay}, num_rnn_layer: ${num_rnn_layer}, dim_question: ${dim_question}, dim_latent: ${dim_latent}, dropout: ${dropout}"
                        echo -e "w_penalty_neg: ${w_penalty_neg}, w_learning: ${w_learning}, w_counter_fact: ${w_counter_fact}, w_que_diff_pred: ${w_que_diff_pred}, w_que_disc_pred: ${w_que_disc_pred}, w_user_ability_pred: ${w_user_ability_pred}, "
                        python F:/code/myProjects/dlkt/example/train/dct.py \
                          --setting_name "our_setting" --dataset_name "${dataset_name}" --data_type "${data_type}" \
                          --train_file_name "${dataset_name}_train_fold_${fold}.txt" --valid_file_name "${dataset_name}_valid_fold_${fold}.txt" --test_file_name "${dataset_name}_test_fold_${fold}.txt" \
                          --optimizer_type "adam" --weight_decay "${weight_decay}" --momentum 0.9 \
                          ...
                          --num_concept 123 --num_question 17751 \
                          --dim_question "${dim_question}" --dim_correct 64 --dim_latent "${dim_latent}" --rnn_type "gru" --num_rnn_layer "${num_rnn_layer}" --dropout "${dropout}" \
                          --w_que_diff_pred "${w_que_diff_pred}" --w_que_disc_pred "${w_que_disc_pred}" --w_user_ability_pred "${w_user_ability_pred}" \
                          --w_penalty_neg "${w_penalty_neg}" --w_learning "${w_learning}" --w_counter_fact "${w_counter_fact}" \
                          --save_model False --debug_mode False --use_cpu False --seed 0
                      done
                    done
                  done
...
} >> F:/code/myProjects/dlkt/example/result_local/dct+cf_our_setting_assist2009_fold_0_ob.txt