...
{
  # weight_decay: 0, num_rnn_layer: 3, dim_question: 64, dim_latent: 128, num_mlp_layer: 2, dropout: 0.1
  for fold in "${folds[@]}"
  do
    echo -e "fold: ${fold}"
    python F:/code/myProjects/dlkt/example/train/dct.py \
      ...
      --dim_question 64 --dim_correct 64 --dim_latent 128 --rnn_type "gru" --num_rnn_layer 3 --num_mlp_layer 2 --dropout 0.1 --que_user_share_proj False \
...
} >> F:/code/myProjects/dlkt/example/result_local/dct_irt_new_not_share_baseline_our_setting_assist2012_save.txt