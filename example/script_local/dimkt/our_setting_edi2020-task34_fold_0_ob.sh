...

{
  ...
  do
    echo -e "weight_decay: 0.001, dim_emb: 128, num_min_question: 1000, num_min_concept: 30, num_diff: 100, dropout: ${dropout}"
    python F:/code/myProjects/dlkt/example/train/dimkt.py \
      ...
      --num_min_question 1000 --num_min_concept 30 --num_concept 53 --num_question 948 \
  ...
  do
    for dropout in ${dropouts}
    do
      echo -e "weight_decay: 0.001, dim_emb: 128, num_min_question: 1000, num_min_concept: 50, num_diff: ${num_diff}, dropout: ${dropout}"
      python F:/code/myProjects/dlkt/example/train/dimkt.py \
        --setting_name "our_setting" --dataset_name "${dataset_name}" --data_type "${data_type}" \
        --train_file_name "${dataset_name}_train_fold_${fold}.txt" --valid_file_name "${dataset_name}_valid_fold_${fold}.txt" --test_file_name "${dataset_name}_test_fold_${fold}.txt" \
        --optimizer_type "adam" --weight_decay 0.001 --momentum 0.9 \
        ...
        --train_batch_size 64 --evaluate_batch_size 256 \
        --enable_clip_grad False --grad_clipped 10.0 \
        --num_min_question 1000 --num_min_concept 50 --num_concept 53 --num_question 948 \
  ...
  do
    for num_min_question in 100 300 500 750 1000
    do
      ...
      do
        ...
        do
          for dropout in ${dropouts}
          do
            echo -e "weight_decay: 0.0001, dim_emb: ${dim_emb}, num_min_question: ${num_min_question}, num_min_concept: ${num_min_concept}, num_diff: ${num_diff}, dropout: ${dropout}"
            python F:/code/myProjects/dlkt/example/train/dimkt.py \
              --setting_name "our_setting" --dataset_name "${dataset_name}" --data_type "${data_type}" \
              --train_file_name "${dataset_name}_train_fold_${fold}.txt" --valid_file_name "${dataset_name}_valid_fold_${fold}.txt" --test_file_name "${dataset_name}_test_fold_${fold}.txt" \
...
} >> F:/code/myProjects/dlkt/example/result_local/dimkt_our_setting_edi2020-task34_fold_0_ob.txt