...

{
  ...
  do
    echo -e "weight_decay: 0.001, dim_emb: 128, num_min_question: 20, num_min_concept: 30, num_diff: 100, dropout: ${dropout}"
    python F:/code/myProjects/dlkt/example/train/dimkt.py \
      ...
      --num_min_question 20 --num_min_concept 30 --num_concept 188 --num_question 11858 \
  ...
  do
    ...
    do
      ...
      do
        for dropout in ${dropouts}
        do
          echo -e "weight_decay: 0.001, dim_emb: 128, num_min_question: ${num_min_question}, num_min_concept: ${num_min_concept}, num_diff: ${num_diff}, dropout: ${dropout}"
          python F:/code/myProjects/dlkt/example/train/dimkt.py \
            --setting_name "our_setting" --dataset_name "${dataset_name}" --data_type "${data_type}" \
            --train_file_name "${dataset_name}_train_fold_${fold}.txt" --valid_file_name "${dataset_name}_valid_fold_${fold}.txt" --test_file_name "${dataset_name}_test_fold_${fold}.txt" \
            --optimizer_type "adam" --weight_decay 0.001 --momentum 0.9 \
