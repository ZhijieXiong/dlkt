...

{
  ...
  echo -e "wight_decay: 0, dim_emb: 64, latent: 256, num_predict_layer: 2, dim_predict_mid: 512, dropout: 0.5"
  ...
  do
    ...
    do
      ...
      do
        ...
        do
          echo -e "wight_decay: 0, dim_emb: 256, latent: ${latent}, num_predict_layer: ${num_predict_layer}, dim_predict_mid: ${dim_predict_mid}, dropout: ${dropout}"
          python F:/code/myProjects/dlkt/example/train/dkt.py \
            --setting_name "${setting_name}" --dataset_name "${dataset_name}" --data_type "${data_type}" \
            --train_file_name "${dataset_name}_train_fold_${fold}.txt" --valid_file_name "${dataset_name}_valid_fold_${fold}.txt" --test_file_name "${dataset_name}_test_fold_${fold}.txt" \
            --optimizer_type "adam" --weight_decay 0 --momentum 0.9 \
            ...
            --enable_clip_grad False --grad_clipped 10.0 \
            --use_concept True --num_concept 865 --num_question 7652 \
            --dim_emb 256 --dim_latent "${latent}" --rnn_type "gru" --num_rnn_layer 1 --dropout "${dropout}" --num_predict_layer "${num_predict_layer}" --dim_predict_mid "${dim_predict_mid}" --activate_type "sigmoid" \
            --save_model False --seed 0
        done
      done
    done
...
} >> F:/code/myProjects/dlkt/example/result_local/dkt_our_setting_xes3g5m_fold_0_ob.txt