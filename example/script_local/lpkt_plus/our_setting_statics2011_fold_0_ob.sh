...
#          --num_concept 27 --num_question 1223 --model_version 3 --ablation_set 1 \
        ...
        do
          ...
          do
            ...
            do
              for dropout in ${dropouts}
              do
                echo -e "gamma: 0.005, weight_decay: ${weight_decay}, dim: ${dim}, w_que_diff_pred: ${w_que_diff_pred}, w_que_disc_pred: ${w_que_disc_pred}, w_penalty_neg: ${w_penalty_neg}, w_learning: ${w_learning}, dropout: ${dropout}"
                python F:/code/myProjects/dlkt/example/train/lpkt_plus.py \
                  --setting_name "our_setting" --dataset_name "${dataset_name}" --data_type "${data_type}" \
                  --train_file_name "${dataset_name}_train_fold_${fold}.txt" --valid_file_name "${dataset_name}_valid_fold_${fold}.txt" --test_file_name "${dataset_name}_test_fold_${fold}.txt" \
                  --optimizer_type "adam" --weight_decay "${weight_decay}" --momentum 0.9 \
                ...
                echo -e "gamma: 0, weight_decay: ${weight_decay}, dim: ${dim}, w_que_diff_pred: ${w_que_diff_pred}, w_que_disc_pred: ${w_que_disc_pred}, w_penalty_neg: ${w_penalty_neg}, w_learning: ${w_learning}, dropout: ${dropout}"
                python F:/code/myProjects/dlkt/example/train/lpkt_plus.py \
                  --setting_name "our_setting" --dataset_name "${dataset_name}" --data_type "${data_type}" \
                  --train_file_name "${dataset_name}_train_fold_${fold}.txt" --valid_file_name "${dataset_name}_valid_fold_${fold}.txt" --test_file_name "${dataset_name}_test_fold_${fold}.txt" \
                  --optimizer_type "adam" --weight_decay "${weight_decay}" --momentum 0.9 \
...
} >> F:/code/myProjects/dlkt/example/result_local/lpkt_plus_v3_our_setting_statics2011_fold_0_ob3.txt