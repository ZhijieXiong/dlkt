...

{
  ...
  do
    echo -e "weight_decay: 0, dim_emb: 256, latent: 256, num_predict_layer: 1, dim_predict_mid: 512, dropout: ${dropout}"
  ...
  do
    for dropout in ${dropouts}
    do
      echo -e "wight_decay: 0, dim_emb: 256, latent: 256, num_predict_layer: 2, dim_predict_mid: ${dim_predict_mid}, dropout: ${dropout}"
      python F:/code/myProjects/dlkt/example/train/dkt.py \
        --setting_name "${setting_name}" --dataset_name "${dataset_name}" --data_type "${data_type}" \
        --train_file_name "${dataset_name}_train_fold_${fold}.txt" --valid_file_name "${dataset_name}_valid_fold_${fold}.txt" --test_file_name "${dataset_name}_test_fold_${fold}.txt" \
        --optimizer_type adam --weight_decay 0 --momentum 0.9 \
...
} >> F:/code/myProjects/dlkt/example/result_local/dkt_our_setting_assist2012_fold_0_ob.txt