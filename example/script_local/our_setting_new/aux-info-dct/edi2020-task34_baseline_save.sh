...

{
  ...
  # weight_decay: 0.000001, weight_aux_emb: 0.5, dim_question: 128, dim_latent: 128, num_rnn_layer: 2, num_mlp_layer: 2, dropout: 0.1
  for fold in "${folds[@]}"
  do
    echo -e "fold: ${fold}"
    python F:/code/myProjects/dlkt/example/train/aux_info_dct.py \
      ...
      --weight_aux_emb 0.5 --dim_question 128 --dim_latent 128 --rnn_type "gru" --num_rnn_layer 2 --num_mlp_layer 2 --dropout 0.1 \
      --save_model True --debug_mode False --use_cpu False --seed 0
  done
} >> F:/code/myProjects/dlkt/example/result_local/aux-info-dct_not-share_baseline_our_setting_new_edi2020-task34_save.txt