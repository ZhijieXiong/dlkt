...

{
  ...
  folds=(0 1 2 3 4)
  # weight_decay: 0.0001, weight_aux_emb: 1, dim_question: 64, dim_latent: 128, num_rnn_layer: 3, num_predict_layer: 2, dropout: 0.3
  for fold in "${folds[@]}"
  do
    echo -e "fold: ${fold}"
    python F:/code/myProjects/dlkt/example/train/aux_info_qdkt.py \
      ...
      --weight_aux_emb 1 --dim_question 64 --dim_latent 128 --rnn_type "gru" --num_rnn_layer 3 --dropout 0.3 \
      --num_predict_layer 2 --dim_predict_mid 128 --activate_type "relu" \
      --save_model True --debug_mode False --use_cpu False --seed 0
  done
} >> F:/code/myProjects/dlkt/example/result_local/aux-info-qdkt_our_setting_new_ednet-kt1_save.txt