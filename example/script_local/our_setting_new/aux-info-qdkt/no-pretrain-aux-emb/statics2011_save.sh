...

{
  ...
  folds=(0 1 2 3 4)
  # weight_decay: 0.000001, weight_aux_emb: 1, dim_question: 128, dim_latent: 128, num_rnn_layer: 2, num_predict_layer: 3, dropout: 0.1
  for fold in "${folds[@]}"
  do
    echo -e "fold: ${fold}"
    python F:/code/myProjects/dlkt/example/train/aux_info_qdkt.py \
      ...
      --weight_aux_emb 1 --dim_question 128 --dim_latent 64 --rnn_type "gru" --num_rnn_layer 1 --dropout 0.1 \
      --num_predict_layer 2 --dim_predict_mid 128 --activate_type "relu" \
      --save_model True --debug_mode False --use_cpu False --seed 0
  done
} >> F:/code/myProjects/dlkt/example/result_local/aux-info-qdkt_our_setting_new_statics2011_save.txt