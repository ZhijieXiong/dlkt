...
class DCT(nn.Module):
    ...
    def get_predict_loss(self, batch, loss_record=None):
        ...
        w_counter_fact = self.params["loss_config"]["counterfactual loss"]
        ...
        if w_counter_fact != 0:
            ...
            for t in range(seq_len - 1):
                ...
                cf_user_ability[:, t] = torch.sigmoid(self.proj_latent2ability(self.dropout(cf_latent.squeeze(1))))
        ...
        if w_learning != 0:
            ...
            if num_sample > 0:
                learn_loss_ = -neg_master_leval.mean()
                learn_loss = learn_loss + learn_loss_
                if loss_record is not None:
                    loss_record.add_loss("learning loss", learn_loss_.detach().cpu().item() * num_sample,
                                         num_sample)
