...
class IDCT(nn.Module):
    ...
    def get_predict_loss(self, batch, loss_record=None):
        ...
        w_diff_stable = self.params["loss_config"].get("diff stable loss", 0)
        ...
        if w_diff_stable != 0:
            noise_strength = self.params["other"]["diff_noise_strength"]
            noise = torch.rand_like(que_difficulty).to(self.params["device"]) * noise_strength * 2 - noise_strength
            que_diff_disturbed = torch.clamp(que_difficulty + noise, 0.0, 1.0)
            inter_func_in_disturbed = (user_ability - que_diff_disturbed) * q2c_mask_table[question_seq[:, 1:]]
            irt_logits_disturbed = que_discrimination.unsqueeze(-1) * inter_func_in_disturbed
            sum_weight_concept_disturbed = torch.sum(que_diff_disturbed * q2c_mask_table[question_seq[:, 1:]], dim=-1,
                                                     keepdim=True) + 1e-6
            predict_score_disturbed = torch.sigmoid(
                torch.sum(irt_logits_disturbed / sum_weight_concept_disturbed, dim=-1)
            )
            predict_score_disturbed = torch.masked_select(predict_score_disturbed, mask_bool_seq[:, 1:])
            diff_stable_loss = nn.functional.mse_loss(predict_score_disturbed, predict_score)
            num_sample = torch.sum(batch["mask_seq"][:, 1:]).item()
            if loss_record is not None:
                loss_record.add_loss("diff stable loss", diff_stable_loss.detach().cpu().item() * num_sample,
                                     num_sample)
            loss = loss + w_diff_stable * diff_stable_loss

        return loss

    def get_user_loss(self, batch, loss_record, user_stage=True):
        encoder_config = self.params["models_config"]["kt_model"]["encoder_layer"]["IDCT"]
        max_que_disc = encoder_config["max_que_disc"]
        dim_emb = encoder_config["dim_emb"]
        w_monotonic = self.params["loss_config"].get("monotonic loss", 0)
        ...
        if user_stage:
            loss_record.add_loss(
                "predict loss of user stage", predict_loss.detach().cpu().item() * num_sample, num_sample
            )
        else:
            loss_record.add_loss(
                "predict loss of question stage", predict_loss.detach().cpu().item() * num_sample, num_sample
            )
        loss = loss + predict_loss
        ...
        if w_mirt != 0:
            ...
            if num_sample > 0:
                ...
                if user_stage:
                    loss_record.add_loss(
                        "mirt loss of user stage", mirt_loss.detach().cpu().item() * num_sample, num_sample
                    )
                else:
                    loss_record.add_loss(
                        "mirt loss of question stage", mirt_loss.detach().cpu().item() * num_sample, num_sample
                    )
                loss = loss + mirt_loss * w_mirt
        ...
        if w_monotonic != 0:
            ...
            if num_sample > 0:
                ...
                if user_stage:
                    loss_record.add_loss(
                        "monotonic loss of user stage", monotonic_loss.detach().cpu().item() * num_sample, num_sample
                    )
                else:
                    loss_record.add_loss(
                        "monotonic loss of question stage", monotonic_loss.detach().cpu().item() * num_sample, num_sample
                    )
                loss = loss + monotonic_loss * w_monotonic

        return loss

    def get_question_loss(self, batch, loss_record, user_stage=False):
        encoder_config = self.params["models_config"]["kt_model"]["encoder_layer"]["IDCT"]
        max_que_disc = encoder_config["max_que_disc"]
        dim_emb = encoder_config["dim_emb"]
        w_mirt = self.params["loss_config"].get("mirt loss", 0)
        ...
        if w_diff_stable != 0:
            ...
            if user_stage:
                loss_record.add_loss(
                    "diff stable loss of user stage", diff_stable_loss.detach().cpu().item() * num_sample, num_sample
                )
            else:
                loss_record.add_loss(
                    "diff stable loss of question stage", diff_stable_loss.detach().cpu().item() * num_sample, num_sample
                )
            loss = loss + w_diff_stable * diff_stable_loss
